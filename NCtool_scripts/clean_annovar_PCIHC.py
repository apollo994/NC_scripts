...
def update_PCICH(chrom, pos, ref, alt, PC_count_dict) :
    ...
    if var in PCdata.index:
        ...
        if isinstance(temp_data, pd.DataFrame):
            ...
            #Lists converted in list of tuples gene_symbol (ex_row[1]) max value (ex_row[2])
            all_gene=[]
            for ex_row in genes_to_test:
                # only gene symbol present in geneDB and higher than threshold
                if check_in_geneDB(ex_row[1]) == True and ex_row[2]>float(PCth):
                    all_gene.append(tuple((ex_row[1], ex_row[2])))
            #List of tuple sorted
            all_gene.sort(key = lambda x: float(x[1]), reverse = True)

            if len(all_gene)==0:
                # print ("Multiple but NOT in DB or under th")
                return (["NOT_in_PC_tab",PC_count_dict])

            if len(all_gene)==1:
                gene_symbol=all_gene[0][0]
                # print ("Multiple but only one in DB and above th: ", gene_symbol)
                PC_count_dict['single'].append([var,gene_symbol])
                return ([gene_symbol,PC_count_dict])

            if len(all_gene)>1:
                number_of_genes=len(all_gene)
                # sublist with only top score gene
                max_gene=[all_gene[0]]
                for gene in all_gene[1:]:
                    if gene[1]==max_gene[0][1]:
                        max_gene.append(gene)

                # ncRVIS HIGHER SCORE VERSION
                if len(max_gene)==1:
                    gene_symbol=max_gene[0][0]
                    # print ("Multiple but ONE top scoring: ", gene_symbol)
                    PC_count_dict['multiple'].append([var,gene_symbol,number_of_genes])
                    return ([gene_symbol,PC_count_dict])
                else:
                    just_geneID=[]
                    for gene in max_gene:
                        just_geneID.append(gene[0])

                    sub_DB=geneDB.loc[geneDB['gene_name'].isin(just_geneID)]
                    sub_DB=sub_DB.set_index('gene_name')
                    sub_DB=sub_DB.sort_values(['ncRVIS', 'pLI'], ascending=[False, False])
                    gene_symbol=sub_DB.index[0]
                    # print ("Multiple, choosen: ", gene_symbol)
                    PC_count_dict['multiple'].append([var,gene_symbol,number_of_genes])
                    return ([gene_symbol,PC_count_dict])
        ...
        #single gene assignament
            ...
            if check_in_geneDB(sym_to_test) == False:
                # print ("In PCHIH but not in geneDB: ", sym_to_test)
                return (["NOT_in_PC_tab",PC_count_dict])
            ...
            if PCdata.loc[var]["max"]<float(PCth):
                ...
                # print ("Only one PCHIH gene: ", gene_symbol)
                PC_count_dict['single'].append([var,gene_symbol])
                return ([gene_symbol,PC_count_dict])
    else:
        # print ("Var NOT in PCHIC: ", var)
        return (["NOT_in_PC_tab",PC_count_dict])
...
with open(inF, 'r') as f, open(outF, 'w') as fo:
        ...
        if ((len(ref) > 1) or (len(alt) > 1)):
            ...
            if any(re.findall(r'exonic|splicing|ncRNA', annovar_annotation, re.IGNORECASE)):
              ...
              pass
                ...
                #START_my_chages################################################
                updated_gene = update_PCICH(chrom, pos, ref, alt, PC_count_dict)
                # if the gene is not reassignment go fot the normal pipeline
                if updated_gene[0]=="NOT_in_PC_tab":
                    PC_count_dict=updated_gene[1]
                    updated_gene = update_intergenic(closest_gene_name, chrom, pos)
                else:
                    PC_count_dict=updated_gene[1]
                    updated_gene=updated_gene[0]
                #END_my_chages##################################################
                ...
                #START_my_chages################################################
                temp_gene = update_PCICH(chrom, pos, ref, alt, PC_count_dict)
                # if the gene is not reassignment use the gene assigned by the normal pipeline
                if temp_gene[0]=="NOT_in_PC_tab":
                    PC_count_dict=temp_gene[1]
                else:
                    PC_count_dict=temp_gene[1]
                    updated_gene=temp_gene[0]
                #END_my_chages##################################################
                local_gene_annotations = geneDB.loc[geneDB["gene_name"] == updated_gene].iloc[0,4:].values
    ...
    #START_my_chages################################################
    total_reassignment=len(PC_count_dict["single"])+len(PC_count_dict["multiple"])

    print("\nResult of the reassignment:")
    print("PCICH gene has been assigned to ",total_reassignment," variants")
    print("single gene= ",len(PC_count_dict["single"]))
    print("multple genes= ",len(PC_count_dict["multiple"]))
    print()

    with open (PC_for_later , "w") as PC_res_out:
        for gene in PC_count_dict["single"]:
            PC_res_out.write(gene[0]+"\t"+gene[1]+"\t"+"1"+"\n")
        for gene in PC_count_dict["multiple"]:
            PC_res_out.write(gene[0]+"\t"+gene[1]+"\t"+str(gene[2])+"\n")

    print('saving in {}'.format(PC_for_later))
